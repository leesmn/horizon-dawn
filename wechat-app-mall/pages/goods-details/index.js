    ...
    if (goodsDetailRes.code == 200) {
      ...
      if (goodsDetailRes.data.properties) {
        ...
        that.setData({
          ...
          selectSizePrice: goodsDetailRes.data.basicInfo.price,
          totalScoreToPay: goodsDetailRes.data.basicInfo.giftPoint
        });
      }
      ...
      let _data = {
        ...
        selectSizePrice: goodsDetailRes.data.basicInfo.price,
        totalScoreToPay: goodsDetailRes.data.basicInfo.giftPoint,
        buyNumMax: goodsDetailRes.data.basicInfo.stock,
        buyNumber: (goodsDetailRes.data.basicInfo.stock > 0) ? 1 : 0,
      ...
      _data.goodsDetail.basicInfo.pics = [];
      var pics = [];
      if (goodsDetailRes.data.basicInfo.albumPics){
        pics = goodsDetailRes.data.basicInfo.albumPics.split(",");
      }else{
        pics.push(goodsDetailRes.data.basicInfo.pic);
      }
      
      for (var i = 0; i < pics.length;i++){
        _data.goodsDetail.basicInfo.pics.push({ "pic": pics[i]});
      }
      that.setData(_data);
      WxParse.wxParse('article', 'html', goodsDetailRes.data.basicInfo.detailHtml, that, 5);
    }
  },
  goShopCar: function() {
    wx.reLaunch({
